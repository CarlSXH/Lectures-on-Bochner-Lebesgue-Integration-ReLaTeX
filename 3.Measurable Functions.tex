...
\begin{definition}
A function, $f$, from $X$ to $B$ is called a \defline{simple $S$-measurable function}\index{simple S-measurable function@simple $S$-measurable function} (or, for brevity, an \defline{$S$-simple function}) if the range of $f$ is a finite set, $\brc{b_1,\dots,b_n}$, in $B$, and if $f^{-1}(b_i)\in S$ for each $b_i\neq0$.
\end{definition}
...
\begin{definition}
A function, $f$, from $X$ to $B$ is said to be \defline{$S$-measurable}\index{S-measurable@$S$-measurable} if there is a sequence, $f_n$, of simple $S$-measurable functions which converges to $f$ pointwise, that is, such that for every $x\in X$ the sequence $f_n(x)$ converges to $f(x)$.
\end{definition}
...
Let $(X,S,\mu)$ be a measure space, and let $P$ be a property which may or may not hold for various points of $X$. Then $P$ is said to hold, or be true, \defline{almost everywhere}\index{almost everywhere} on $X$ (abbreviated a.e.) if there is a null set $N$ for $\mu$ such that the property $P$ is true for all points in $X$ except possibly those in $N$.
\end{definition}

This definition will be illustrated in Definition \ref{def:mu measurable function}.

\begin{definition}
A function, $f$, from $X$ to $B$ is called a \defline{simple $\mu$-measurable function}\index{simple mu-measurable function@simple $\mu$-measurable function} if $f$ is a simple $(S\du\nring\mu)$-measurable function. (The definition of $S\du\nring\mu$ was given in Theorem \ref{thm:completion of general measure}.)
\end{definition}

\begin{definition}\label{def:mu measurable function}
A function, $f$, with values in $B$, which is defined almost everywhere on $X$, is said to be \defline{$\mu$-measurable}\index{mu-measurable@$\mu$-measurable} if there is a sequence, $f_n$, of simple $\mu$-measurable functions which converges pointwise to $f$ almost everywhere.
\end{definition}
...
\begin{definition}
If $f$ is a function from $X$ to $B$, then we define the \defline{carrier}\index{carrier} of $f$ to be $\brc{x\in X: f(x)\neq 0}$. We will denote the carrier of $f$ by $\car{f}$. 
\end{definition}
...
\begin{theorem}[Egoroff (1911)]\label{thm:egoroff}\index{Egoroff's Theorem}
...
\begin{definition}
A sequence $f_n$ of functions is said to be \defline{Cauchy in measure}\index{Cauchy in measure} on $E\subseteq X$ if the intersection of $E$ with each $\car{f_n}$ is in the domain of $\mu^*$, and if for every $\ep>0$ \[\lim_{m,n\to+\infty}\mu^*(\brc{x\in X:\norm{f_m(x)-f_n(x)}\geq\ep})=0.\]
\end{definition}
...
\begin{theorem}[Riesz-Weyl]\label{thm:riesz weyl}\index{Riesz-Weyl Theorem}
...
\begin{definition}
Let $\mu$ be an arbitrary measure on a measurable space $(X, S)$, and let $E\subseteq X$. Then a sequence, $f_n$, of functions will be said to converge on $E$ to a function $f$ \defline{almost uniformly}\index{almost uniformly} (resp. \defline{in measure}\index{almost uniformly in measure}) with respect to $\mu$ if $f_n$ converges to $f$ on $E$ almost uniformly (resp. in measure) with respect to $|\mu|$. We define what is meant by a sequence of functions which is \defline{almost uniformly Cauchy}\index{almost uniformly Cauchy} (resp. \defline{Cauchy in measure}\index{almost uniformly Cauchy in measure}) with respect to $\mu$, in analogous fashion. 
\end{definition}
