...
\begin{lemma}
\label{lem:triangle inequality ISF integral}
...
\begin{proposition}
\label{prop:integral small outside finite set}
...
\begin{theorem}
\label{thm:bochner characterization integrable}
...
\item Let $\mu$ be Lebesgue measure on $[0,1]$, and let $f_1,f_2,\dots$ be the characteristic functions of the intervals $[0,1/2],[1/2,1],[0,1/3],[1/3,2/3],[2/3,1],[0,1/4],\dots$ (you may have discovered this sequence of sets in answer to Exercise \ref{exer:item:in measure not ae} of Chapter \ref{ch:meas func}). Let $f$ be the function on $[0,1]$ with values in $\ell^\infty$ whose value at $t\in[0,1]$ is the sequence $(f_1(t),f_2(t),\dots)$
