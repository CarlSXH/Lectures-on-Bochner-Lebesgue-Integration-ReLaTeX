...
\begin{definition}
For $0<p<\infty$ we will denote the set of all $B$-valued $\mu$-measurable functions, $f$, such that $\norm{f(\imarg)}^p$ is integrable, by $\cL^p(X,S,\mu,B)$\index{Lp space@$L^p$ space} (or suitable abbreviations thereof). We define a real-valued function, $\norm{\imarg}_p$, on $\cL^p$ by \[\norm{f}_p=\br{\int\norm{f(x)}^p\dd|\mu|(x)}^{1/p}.\] For $1\leq p<\infty$ this function is called the \defline{$L^p$-norm}\index{Lp-norm@$L^p$-norm}.
\end{definition}
...
\begin{proposition}\label{prop:zero Lp norm}
...
\begin{lemma}\label{lem:pq inequality}
...
\begin{theorem}[H\"older's inequality]\label{thm:holders}\index{Holder's inequality@H\"older's inequality}
...
\begin{theorem}[Minkowski's inequality]\label{thm:minkowski inequality}\index{Minkowski's inequality}
...
\begin{definition}
For $1<p<\infty$ the normed space consisting of the equivalence classes of functions in $\cL^p$ which agree a.e. will be denoted by $L^p(X,S,\mu,B)$\index{Lp space@$L^p$ space} (or appropriate abbreviations thereof). We will denote the norm on $L^p$ again by $\norm{\imarg}_p$ and we will call it the \defline{$L^p$-norm}\index{Lp-norm@$L^p$-norm}.
\end{definition}
...
\begin{definition}
Convergence with respect to the $L^p$-norm will be called \defline{convergence in $p$-mean}\index{convergence in p-mean@convergence in $p$-mean}. A sequence of functions in $\cL^p$ which is a Cauchy sequence with respect to the $L^p$-norm will be called a \defline{$p$-mean Cauchy sequence}\index{p-mean Cauchy sequence@$p$-mean Cauchy sequence}.
\end{definition}
...
This inequality is known as the Markov inequality\index{Markov inequality}, and for the case $p=2$ it is also called the Tchebychev inequality\index{Tchebychev inequality}.
...
\begin{corollary}\label{cor:converge in p mean is inside Lp}
...
\begin{theorem}\label{thm:ISF dense in Lp}
...
\begin{theorem}[Lebesgue Dominated Convergence Theorem]\index{Lebesgue Dominated Convergence
Theorem}
...
\begin{theorem}[The Monotone Convergence Theorem]\label{thm:mct for Lp}\index{Monotone Convergence Theorem}
...
\begin{proposition}\label{prop:set diff union are cts}
...
\begin{definition}
Let $f$ be a vector-valued function on a topological space $X$. By the \defline{support}\index{support} of $f$ is meant the smallest closed set outside of which the function everywhere has value $0$.
\end{definition}

\begin{theorem}\label{thm:cpct support dense in Lp}
